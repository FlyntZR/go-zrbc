...
func GetFileKeyNew(fSrc []byte) string {
	...
	fileKey := uuid.New().String() + "." + fileType
	return fileKey
}

func GetFileKeyNewNew(orgFileName string) string {
	strs := strings.Split(orgFileName, ".")
	if len(strs) > 0 {
		tStr := strings.ToLower(strs[len(strs)-1])
		if tStr == "jpg" || tStr == "jpeg" || tStr == "png" || tStr == "mp4" || tStr == "m3u8" || tStr == "mobileconfig" || tStr == "apk" {
			return fmt.Sprintf("%s.%s", uuid.New().String(), strs[len(strs)-1])
		}
	}
